...
cc_options_t cc_options = { .print_ir = 0, .ir_file = NULL, .ir_force = 0, .input_file = NULL,
      .output_file = NULL };
...
void print_usage(const char *prg_name) {
	fprintf(stderr, "Usage: %s [-p] [-f] [-o output] [-h] file\n", prg_name);
}
...
int process_options(int argc, char *argv[]) {
	...
	while ((opt = getopt(argc, argv, "hpfo:")) != -1) {
		switch (opt) {
		case 'p':
			cc_options.print_ir = 1;
			break;
		case 'f':
			cc_options.ir_force = 1;
			break;
		case 'o':
			/* output file */
				...
				if (cc_options.ir_file == NULL) {
					...
					if (cc_options.ir_force == 0) {
						...
						fprintf(f,
						      "/****************************************************************************/\n");
						fprintf(f,
						      " * This file was generated by the forty-two yacp compiler.                  * \n");
						fprintf(f,
