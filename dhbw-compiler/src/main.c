...
void print_usage(const char *prg_name) {
	fprintf(stderr, "Usage: %s [-p] [-f] [-d debug] [-o output] [-h] file\n", prg_name);
}
...
int process_options(int argc, char *argv[]) {
	...
	while ((opt = getopt(argc, argv, "hpfd:o:")) != -1) {
		switch (opt) {
		case 'p':
		...
		case 'd':
			/* debug logfile */
			if (optarg != NULL) {
				cc_options.dbg = strdup(optarg);
				if (cc_options.dbg == NULL) {
	...
	if (optind >= argc) {
		...
		// Create a new file if not exists. When force is set to 0 check if file already exists and fail it does.
		if (cc_options.dbg != NULL) {
			if ((strcmp(cc_options.dbg, "stdout") == 0) || (strcmp(cc_options.dbg, "stderr") == 0)) {
...
void debug(char *string, ...) {
	if (cc_options.dbg != NULL) {
		FILE* f;
		if (strcmp(cc_options.dbg, "stdout") == 0) {
			f = stdout;
		} else if (strcmp(cc_options.dbg, "stderr") == 0) {
			f = stderr;
		} else
			f = fopen(cc_options.dbg, "a+");
		va_list ap;
		va_start(ap, string);
		vfprintf(f, string, ap);
		fprintf(f, "\n");
		if (!strcmp(cc_options.dbg, "stdout") && !strcmp(cc_options.dbg, "stderr"))
			fclose(f);
	}
}
